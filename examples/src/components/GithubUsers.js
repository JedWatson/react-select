...
const GithubUsers = React.createClass({
	...
	getUsers (input) {
		return fetch(`https://api.github.com/search/users?q=${input}`)
			...
			.then((json) => {
				if(json.items) {
					return { options: json.items.map( (item) => ( { ...item, href: item.html_url } )) };
				} else {
					return { options: [] };
				}
      }		);
	...
	render () {
		return (
			<div className="section">
				...
				<Select.Async multi={this.state.multi} value={this.state.value} onChange={this.onChange} valueKey="id" labelKey="login" loadOptions={this.getUsers} minimumInput={1} backspaceRemoves={false} />
