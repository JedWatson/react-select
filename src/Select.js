...
class Select extends React.Component {
	...
	componentWillMount() {
		...
		if (this.props.required) {
			this.setState({
				required: Select.handleRequired(valueArray[ 0 ], this.props.multi),
			});
		}
	}
	...
	componentWillReceiveProps(nextProps) {
		...
		if (nextProps.required) {
			this.setState({
				required: Select.handleRequired(valueArray[ 0 ], nextProps.multi),
			});
		}
	}
	...
	getValueArray(value, nextProps) {
		...
		if (props.multi) {
			...
			return value.map(value => Select.expandValue(value, props)).filter(i => i);
		}
		const expandedValue = Select.expandValue(value, props);
	...
	setValue(value) {
		...
		if (this.props.required) {
			const required = Select.handleRequired(value, this.props.multi);
	...
	renderClear() {
		...
		return (
			<span
				...
				children={this.props.clearRenderer()}
			/>
		);
	}
	...
	renderArrow() {
		...
		return (
			<span
				...
				children={this.props.arrowRenderer({ onMouseDown, isOpen })}
			/>
		);
	}
	...
	render() {
		...
		const isOpen = this.props.multi && !options.length && valueArray.length && !this.state.inputValue
			? false
			: this.state.isOpen;

		let focusedOption = focusedOptionIndex !== null
...
Select.handleRequired = (value, multi) => value ? multi ? value.length === 0 : Object.keys(value).length === 0 : true;

Select.expandValue = (value, props) => {
	const valueType = typeof value;
